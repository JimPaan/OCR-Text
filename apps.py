...
def detect_intent(input_text):
    """Classify the user's intent based on input text."""
    ...
    if choice == "Home":
        ...
        if user_input:
            with st.spinner("Processing..."):
                ...
                if intent == "context_query" and ocr_text:
                    response = generate_response(ocr_text, user_input)
                else:
                    response = random.choice(
                        [intent_dict["responses"] for intent_dict in intents if intent_dict["tag"] == intent][0])

            st.text_area("Chatbot:", value=response, height=300, max_chars=None, key=f"chatbot_response_{counter}")
            ...
            with open('chat_log.csv', 'a', newline='', encoding='utf-8') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow([str(user_input), response, timestamp])
